    ...
    <Row>
        ...
        <Col md={4}>
            <Card>
                <ListGroup variant='flush'>
                    <ListGroup.Item>
                        <h2>Podsumowanie ({ cartItems.reduce((acc, item) => acc+item.qty, 0) })
                        przedmiot/-ów
                        </h2>
                        { cartItems.reduce((acc, item) => acc+item.qty*item.price, 0).toFixed(2) } zł
                    </ListGroup.Item>
                    <ListGroup.Item>
                        <Button type='button' className='btn-block' disabled={cartItems.length === 0} onClick={ checkoutHandler }> Przejdź do kasy </Button>          
                    </ListGroup.Item>
                </ListGroup>
            </Card>
        </Col>   
